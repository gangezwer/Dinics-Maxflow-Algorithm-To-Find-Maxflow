...
public class DincsSolver extends Base {
    ...
    //ArrayList will contain node indexes, to track the path from Source to Sink to print in console.
    ...
    public void solve() {
        ...
        while (bfs()){
            ...
            for(long f= dfs(s,index,INF); f!=0; f=dfs(s,index,INF)){
                ...
                //Initializing with new Arraylist in order to store the new Path.
                path = new ArrayList<>();
            }
        }
    ...
    //Printing the level of the node of the flow network graph
